<x-app-layout>
  <x-page-header title="Daftar Challenge Submission" subtitle="Lihat semua pengumpulan challenge mahasiswa" />
        ...
        <table class="min-w-full border border-gray-200 text-sm">
          <thead class="bg-gray-100">
            <tr>
              ...
              <th class="border px-3 py-2">Challenge</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="border px-3 py-2">{{ $submission->challenge->judul ?? '-' }}</td>
              <td class="border px-3 py-2">{{ $submission->mhs->nama ?? '-' }}</td>
              <td class="border px-3 py-2">{{ ucfirst($submission->status) }}</td>
              <td class="border px-3 py-2">{{ $submission->challenge->xp ?? 0 }}</td>
              ...
              <td class="border px-3 py-2 space-x-1">
                <x-button btn="warning"
                  onclick="window.location='{{ route('challenge_submission.edit', $submission->id) }}'">Edit</x-button>
                <form action="{{ route('challenge_submission.destroy', $submission->id) }}" method="POST"
